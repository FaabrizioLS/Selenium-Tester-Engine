...
def run_input_color_tests(driver): # Recibimos un driver Selenium ya inicializado
    ...
    def probar_input(input_color, identificador):
        ...
        def color_valido():
            input_color.clear()
            input_color.send_keys("#00FF00")
            valor = input_color.get_attribute("value")
            return ("Color aceptado correctamente." if valor.upper() == "#00FF00" else f"No aceptado: '{valor}'", "✔")

        def color_invalido():
            input_color.clear()
            input_color.send_keys("123456")
            valor = input_color.get_attribute("value")
            return ("Formato inválido rechazado." if valor != "123456" else "Aceptó formato inválido.", "⚠")
        ...
        def xss():
            input_color.clear()
            input_color.send_keys("<script>alert(1)</script>")
            valor = input_color.get_attribute("value")
            return ("XSS sin sanitizar." if '<' in valor else "XSS bloqueado.", "❗" if '<' in valor else "✔")

        def sql():
            input_color.clear()
            input_color.send_keys("'#ff0000' OR '1'='1")
            valor = input_color.get_attribute("value")
